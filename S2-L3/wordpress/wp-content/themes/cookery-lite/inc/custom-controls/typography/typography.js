 ...
 */
wp.customize.controlConstructor['cookery-lite-typography'] = wp.customize.Control.extend( {
    ready: function(){        
        'use strict';
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.cookery-lite-variant-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.cookery-lite-variant-wrapper' ).css( 'display', 'block' );
				}
