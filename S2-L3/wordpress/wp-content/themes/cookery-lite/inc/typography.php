...
if( ! function_exists( 'cookery_lite_fonts_url' ) ):
/**
 * 
*/ 
function cookery_lite_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'cookery-lite' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'cookery-lite' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'cookery-lite' );
...
if( ! function_exists( 'cookery_lite_get_google_fonts' ) ) :
/**
 * Get Google Fonts
*/
function cookery_lite_get_google_fonts(){
...
if( ! function_exists( 'cookery_lite_get_websafe_font' ) ) :
/**
 * Function listing WebSafe Fonts and its attributes
*/
function cookery_lite_get_websafe_font(){
    ...
    return apply_filters( 'cookery_lite_standard_fonts', $standard_fonts );
}
endif;

if( ! function_exists( 'cookery_lite_check_varient' ) ) :
/**
 * Checks for matched varients in google fonts for typography fields
*/
function cookery_lite_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = cookery_lite_get_google_fonts(); //Google Fonts
    $websafe_fonts = cookery_lite_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function cookery_lite_get_css_variant( $font_variant ){
    $v_array = array(
		'100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function cookery_lite_is_google_font( $font ){
    $return = false;
    $websafe_fonts = cookery_lite_get_websafe_font();
    if( $font ){
...
*/
function cookery_lite_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = cookery_lite_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( cookery_lite_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = cookery_lite_get_css_variant( cookery_lite_check_varient( $font_family, $font_variant ) );
...
if( ! function_exists( 'cookery_lite_get_all_fonts' ) ) :
/**
 * Return Web safe font and google font without variants
*/
function cookery_lite_get_all_fonts(){
    $google = array();        
    $standard = apply_filters( 'cookery_lite_standard_font', array(
		'georgia-serif'       => __( 'Georgia', 'cookery-lite' ),
        'palatino-serif'      => __( 'Palatino Linotype, Book Antiqua, Palatino', 'cookery-lite' ),
        'times-serif'         => __( 'Times New Roman, Times', 'cookery-lite' ),
        'arial-helvetica'     => __( 'Arial, Helvetica', 'cookery-lite' ),
        'arial-gadget'        => __( 'Arial Black, Gadget', 'cookery-lite' ),
		'comic-cursive'       => __( 'Comic Sans MS, cursive', 'cookery-lite' ),
		'impact-charcoal'     => __( 'Impact, Charcoal', 'cookery-lite' ),
        'lucida'              => __( 'Lucida Sans Unicode, Lucida Grande', 'cookery-lite' ),
        'tahoma-geneva'       => __( 'Tahoma, Geneva', 'cookery-lite' ),
		'trebuchet-helvetica' => __( 'Trebuchet MS, Helvetica', 'cookery-lite' ),
		'verdana-geneva'      => __( 'Verdana, Geneva', 'cookery-lite' ),
        'courier'             => __( 'Courier New, Courier', 'cookery-lite' ),
        'lucida-monaco'       => __( 'Lucida Console, Monaco', 'cookery-lite' ),
	) );
